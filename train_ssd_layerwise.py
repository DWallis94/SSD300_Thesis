...
tf.app.flags.DEFINE_integer(
    "num_preprocessing_threads", 24, "The number of threads used to create the batches."
)
tf.app.flags.DEFINE_integer(
    "num_cpu_threads", 0, "The number of cpu cores used to train."
)
tf.app.flags.DEFINE_float("gpu_memory_fraction", 1.0, "GPU memory fraction to use.")
# scaffold related configuration
tf.app.flags.DEFINE_string(
    "data_dir",
    "../VOCROOT_backup/tfrecords",
    "The directory where the dataset input data is stored.",
)

tf.app.flags.DEFINE_string(
    "model_dir", "./logs/", "The directory where the model will be stored."
)
tf.app.flags.DEFINE_integer(
    "log_every_n_steps", 10, "The frequency with which logs are printed."
)
tf.app.flags.DEFINE_integer(
    "save_summary_steps",
    500,
...
tf.app.flags.DEFINE_integer(
    "train_image_size", 300, "The size of the input image for the model to use."
)
tf.app.flags.DEFINE_integer(
    "train_epochs", None, "The number of epochs to use for training."
)
tf.app.flags.DEFINE_integer(
    "max_number_of_steps", 120000, "The max number of steps to use for training."
)
tf.app.flags.DEFINE_integer("batch_size", 32, "Batch size for training and evaluation.")
tf.app.flags.DEFINE_string(
    "data_format",
    "channels_first",  # 'channels_first' or 'channels_last'
...
tf.app.flags.DEFINE_string(
    "checkpoint_path", "./model", "The path to a checkpoint from which to fine-tune."
)
tf.app.flags.DEFINE_string(
...
tf.app.flags.DEFINE_boolean(
    "multi_gpu", True, "Whether there is GPU to use for training."
)
...
tf.app.flags.DEFINE_integer(
    "qw_bits", 32, "Number of quantization bits to allocate to the network weights."
)
tf.app.flags.DEFINE_integer(
    "qa_bits", 32, "Number of quantization bits to allocate to the network activations."
)
...
tf.app.flags.DEFINE_float(
    "threshold_w", 0, "Pruning threshold under which to zero out the weights to."
)
tf.app.flags.DEFINE_float(
    "threshold_a", 0, "Pruning threshold under which to zero out the activations."
)
tf.app.flags.DEFINE_integer(
    "begin_pruning_at_step",
    20000,
    "Specifies which step pruning will begin to occur after.",
)
tf.app.flags.DEFINE_integer(
    "end_pruning_at_step", 100000, "Specifies which step pruning will end after."
)
tf.app.flags.DEFINE_integer(
    "pruning_frequency", 1000, "Specifies how often to prune the network."
)
tf.app.flags.DEFINE_float(
    "target_sparsity",
    0.5,
...
tf.app.flags.DEFINE_integer(
    "num_classes",
    len(dataset_common.VOC_LABELS_reduced),
...
def input_pipeline(
    ...
    def input_fn():
        ...
        global_anchor_info = {
            "decode_fn": lambda pred: anchor_encoder_decoder.decode_all_anchors(
...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(
        params["model_scope"], default_name=None, values=[features], reuse=tf.AUTO_REUSE
    ):
        ...
        cls_pred = [
            tf.reshape(pred, [tf.shape(features)[0], -1, params["num_classes"]])
            for pred in cls_pred
        ]
        location_pred = [
            tf.reshape(pred, [tf.shape(features)[0], -1, 4]) for pred in location_pred
        ]
        ...
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope("post_forward"):
                ...
                predictions_for_bg = tf.nn.softmax(
                    tf.reshape(
                        cls_pred, [tf.shape(features)[0], -1, params["num_classes"]]
                    )
                )[:, :, 0]
                prob_for_negtives = tf.where(
                    batch_negtive_mask,
                    0.0 - predictions_for_bg,
                    # ignore all the positives
                    0.0 - tf.ones_like(predictions_for_bg),
                )
                ...
                final_mask = tf.stop_gradient(
                    tf.logical_or(
                        tf.reshape(
                            tf.logical_and(batch_negtive_mask, selected_neg_mask), [-1]
                        ),
                        positive_mask,
                    )
                )
                ...
                flaten_cls_targets = tf.boolean_mask(
                    tf.clip_by_value(flaten_cls_targets, 0, params["num_classes"] - 1),
                    final_mask,
                )
                ...
                predictions = {
                    ...
                    "probabilities": tf.reduce_max(
                        tf.nn.softmax(cls_pred, name="softmax_tensor"), axis=-1
                    ),
    ...
    cross_entropy = tf.losses.sparse_softmax_cross_entropy(
        labels=flaten_cls_targets, logits=cls_pred
    ) * (params["negative_ratio"] + 1.0)
    ...
    loc_loss = modified_smooth_l1(location_pred, flaten_loc_targets, sigma=1.0)
    ...
    total_loss = tf.add(
        cross_entropy + loc_loss,
        tf.multiply(params["weight_decay"], tf.add_n(l2_loss_vars), name="l2_loss"),
        name="total_loss",
    )
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        lr_values = [
            params["learning_rate"] * decay for decay in params["lr_decay_factors"]
        ]
...
def main(_):
    ...
    run_config = (
        ...
        .replace(save_checkpoints_secs=FLAGS.save_checkpoints_secs)
        .replace(save_checkpoints_steps=FLAGS.save_checkpoints_steps)
        .replace(save_summary_steps=FLAGS.save_summary_steps)
        .replace(keep_checkpoint_max=5)
        .replace(tf_random_seed=FLAGS.tf_random_seed)
        .replace(log_step_count_steps=FLAGS.log_every_n_steps)
