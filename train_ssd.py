...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        if FLAGS.low_precision:
            backbone = ssd_net_low.VGG16Backbone(n_boxes=all_num_anchors_depth, n_classes=FLAGS.num_classes, feature_scale=FLAGS.feature_scale, training=(
                mode == 'TRAIN'), data_format=params['data_format'])
            ...
            backbone = ssd_net_high.VGG16Backbone(n_boxes=all_num_anchors_depth, n_classes=FLAGS.num_classes, feature_scale=FLAGS.feature_scale, training=(
                mode == 'TRAIN'), data_format=params['data_format'])
            location_pred, cls_pred = backbone.forward(inputs)
    ...
    with tf.device('/cpu:0'):
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                # include both selected negative and all positive examples
                ...
                model = tf.keras.Model(inputs=inputs, outputs=[tf.argmax(
                    cls_pred, axis=-1), tf.reduce_max(tf.nn.softmax(cls_pred, name='softmax_tensor'), axis=-1), bboxes_pred])
