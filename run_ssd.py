...
tf.app.flags.DEFINE_string(
    "checkpoint_path", "./logs", "The path to a checkpoint from which to fine-tune."
)
tf.app.flags.DEFINE_string(
    "model_scope",
    "ssd300",
...
tf.app.flags.DEFINE_string(
    "input_data",
    "../VOCROOT_reduced/real_dataset/",
    "Specify the location of the input dataset to apply the trained network.",
)
tf.app.flags.DEFINE_string(
    "output_data", "./output/", "Specify the location to output the labelled images."
)
...
def clip_bboxes(ymin, xmin, ymax, xmax, name):
    with tf.name_scope(name, "clip_bboxes", [ymin, xmin, ymax, xmax]):
        ...
        ymax = tf.minimum(ymax, 1.0)
        xmax = tf.minimum(xmax, 1.0)
...
def sort_bboxes(scores_pred, ymin, xmin, ymax, xmax, keep_topk, name):
    with tf.name_scope(name, "sort_bboxes", [scores_pred, ymin, xmin, ymax, xmax]):
        ...
        scores, idxes = tf.nn.top_k(
            scores_pred, k=tf.minimum(keep_topk, cur_bboxes), sorted=True
        )

        ymin, xmin, ymax, xmax = (
            tf.gather(ymin, idxes),
            tf.gather(xmin, idxes),
            tf.gather(ymax, idxes),
            tf.gather(xmax, idxes),
        )
        ...
        return (
            tf.pad(ymin, paddings_scores, "CONSTANT"),
            tf.pad(xmin, paddings_scores, "CONSTANT"),
            tf.pad(ymax, paddings_scores, "CONSTANT"),
            tf.pad(xmax, paddings_scores, "CONSTANT"),
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(
            FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE
        ):
