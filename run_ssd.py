...
def write_images_with_bboxes(image_input, all_labels, all_scores, all_bboxes, shape_input, in_file=FLAGS.input_data,out_file=FLAGS.output_data):
    ...
    with tf.Session() as sess:
            ...
            filename_queue = tf.gfile.ListDirectory(os.path.join(FLAGS.input_data,dataset_lib[0]))
...
def main(_):
    with tf.Graph().as_default():
        ...
        if not FLAGS.just_labels:
            write_images_with_bboxes(image_input=image_input, all_labels=all_labels, all_scores=all_scores, all_bboxes=all_bboxes, shape_input=shape_input)
        write_labels_to_file(image_input=image_input, all_labels=all_labels, all_scores=all_scores, all_bboxes=all_bboxes, shape_input=shape_input)
